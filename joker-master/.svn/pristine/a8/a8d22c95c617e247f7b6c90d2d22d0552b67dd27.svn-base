module localDB {
    export function InitMissionTemps(argList:any[]):void {
    ...
    export class MissionTemplateVO {
        id:number = 1;                  //序列
        missionName:string = "";        //任务名称
        descript:string = "";           //描述
        reward:string = "";             //显示的奖励
        type:number = 1;                //任务类型
        subType:number = 1;             //任务组id
        level:number = 1;               //任务组子级id
        total:number = 1;               //任务需要完成的次数
        imgName:string = "";            //显示的图片
        itemId:number[] = [];           //奖励的道具id
        weekDay:number = 0;             //周几
        constructor(id:number, missionName:string, desc:string, reward:string, type:number, subType:number, level:number, total:number,imgName:string,itemId:number[],weekDay:number) {
            this.id = id;
