var __reflect = (this && this.__reflect) || function (p, c, t) {
    p.__class__ = c, t ? t.push(c) : t = [c], p.__types__ = p.__types__ ? t.concat(p.__types__) : t;
};
var __extends = (this && this.__extends) || function (d, b) {
    for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p];
    function __() { this.constructor = d; }
    d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
};
/**
 * Created by JiangTao on 2016/4/28.
 */
var svrDebug;
(function (svrDebug) {
    var DebugConnectCommands = (function (_super) {
        __extends(DebugConnectCommands, _super);
        function DebugConnectCommands() {
            return _super.call(this) || this;
        }
        DebugConnectCommands.prototype.execute = function (notification) {
            var action = notification.getName();
            switch (action) {
                case app.NetAction.DEBUG_CMDT_CONNECT:
                    cy.resetWA();
                    var pack = new cyvos.SrsPackage();
                    pack.data.writeUnsignedInt(1);
                    pack.sXYID = parseInt(app.NetAction.DEBUG_CMDT_ENCRYPTVER);
                    cy.srsServer.send(pack);
                    return;
                case app.NetAction.DEBUG_CMDT_ENCRYPTVER:
                    var pack = new cyvos.SrsPackage();
                    pack.sXYID = parseInt(app.NetAction.DEBUG_CMDT_REQKEY);
                    cy.srsServer.send(pack);
                    return;
                case app.NetAction.DEBUG_CMDT__RESPKEY:
                    var pack = notification.getBody();
                    var cbKeys = new egret.ByteArray();
                    var cbLen = pack.data.readByte();
                    pack.data.readBytes(cbKeys, 0, cbLen);
                    var keyBV = new Uint8Array(cbKeys.buffer);
                    cy.setKeyWA(gameabc.U8Array.parse(keyBV));
                    return;
            }
        };
        return DebugConnectCommands;
    }(puremvc.SimpleCommand));
    svrDebug.DebugConnectCommands = DebugConnectCommands;
    __reflect(DebugConnectCommands.prototype, "svrDebug.DebugConnectCommands");
})(svrDebug || (svrDebug = {}));
//# sourceMappingURL=DebugConnectCommands.js.map