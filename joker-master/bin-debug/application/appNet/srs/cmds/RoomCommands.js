...
(function (app) {
     ...
     */
    var RoomCommands = (function (_super) {
        __extends(RoomCommands, _super);
        function RoomCommands() {
            return _super.apply(this, arguments) || this;
        }
        RoomCommands.prototype.sendHandler = function (data, stream) {
        ...
        RoomCommands.prototype.callReadyLater = function () {
            this.sendNotification(app.NetAction.REQ_COIN);
            // __PVO().to(app.NetAction.GAME_READY);//在RESP_COIN中
        };
        RoomCommands.prototype.resultHandler = function (stream) {
            switch (this.action) {
                //响应玩家进入房间
                case app.NetAction.RE_JOIN_ROOM:
                    ...
                    if (flag == 0) {
                        ...
                        if (user.getProxy().currentRoom != null) {
                            cy.log("join success ofsid:" + user.getProxy().currentRoom.svrOfsId, 16 /* ROOM */);
                        }
                        //进入房间成功
                        ...
                        if (flag == 1) {
                            ...
                            mc2sdk.event(54003 /* ROOM_FAIELD */, msgBox.szText);
                            tip.popSysCenterTip(msgBox.szText, tip.TIPS_TYPE.TIPS_WARNING);
                        }
                        else {
                            mc2sdk.event(54003 /* ROOM_FAIELD */, flag);
                ...
                case app.NetAction.RE_TABLE_INFO:
                    var tableInfo = stream.getSuruct(cyvos.TableInfo);
                ...
                case app.NetAction.RE_USER_INFO:
                    var playerInfo = stream.getSuruct(cyvos.PlayerInfo);
                ...
                case app.NetAction.RE_ROOM_STATE:
                    if (user.getProxy().currentRoom && user.getProxy().currentRoom.isVip) {
                        var stateInfo = stream.getSuruct(cyvos.PlayerStateInfo);
                ...
                case app.NetAction.RE_JOIN_ROOM_COMPLETE:
                    cy.log("join complete ofsid:" + user.getProxy().currentRoom.svrOfsId, 16 /* ROOM */);
                    // 如果是排队机制
                    if (room.getProxy().isWaitingQueue) {
                        this.sendNotification(app.NetAction.ROOM_ACTION, [1]);
                    }
                    else if (room.getProxy().isMatch) {
                        this.sendNotification(app.NetAction.JOIN_MATCH_GAME);
                    }
                    else if (room.getProxy().currentType == 7 /* PK */) {
                    }
                    this.sendNotification(app.NetAction.ROOM_ACTION, [1, 0, 1]);
                    return;
                //响应玩家动作
                case app.NetAction.RE_ROOM_ACTION:
                    ...
                    if (flag == 1) {
                        var msgBox = stream.getSuruct(cyvos.MsgBox);
                        mc2sdk.event(54004 /* ROOM_ACT_FAILED */, msgBox.szText);
                        tip.popSysCenterTip(msgBox.szText, tip.TIPS_TYPE.TIPS_WARNING);
                        cy.log(msgBox.szText, 32 /* UNDEFINE */);
                    }
                    else if (flag != 0) {
                        mc2sdk.event(54004 /* ROOM_ACT_FAILED */, flag);
                    }
                ...
                case app.NetAction.NOTICE_CLIENT_ACTION:
                    ...
                    return;
                case app.NetAction.NOTICE_CLIENT_WAITING_LIST:
                    ...
                    else if (room.getProxy().currentType != 9 /* GRIL */) {
                        user.getProxy().leaveRoom();
                    }
                    return;
                //离开房间成功
                case app.NetAction.RE_LEAVE_ROOM:
                    if (user.getProxy().currentRoom == null) {
                        cy.log("leave success ofsid:unknown.", 16 /* ROOM */);
                    }
                    else {
                        cy.log("leave success ofsid:" + user.getProxy().currentRoom.svrOfsId, 16 /* ROOM */);
                    }
                ...
                case app.NetAction.CHAT_MSG:
                    var channel = stream.getByte();
                    var color = stream.getInt();
                    var areaid = stream.getInt();
                    var numberid = stream.getInt();
                    var msg_str = stream.getStr();
                ...
                case app.NetAction.RESP_COIN:
                    ...
                    __PVO().to(app.NetAction.GAME_READY);
                    return;
                case app.NetAction.RESP_PLAYER_RANK:
                    var flag = stream.getByte();
    ...
    app.RoomCommands = RoomCommands;
    __reflect(RoomCommands.prototype, "app.RoomCommands");
})(app || (app = {}));
